...
from llava.mm_utils import tokenizer_image_token, map_obj, PlainBoxFormatter
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value'] or DEFAULT_VIDEO_TOKEN in sentence['value']:
                ...
                sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()  # 这里 strip 会把 \n 删除
                sentence['value'] = DEFAULT_IMAGE_TOKEN + '\n' + sentence['value']
                sentence['value'] = sentence['value'].strip()  # 这里没有影响, \n 换行符保留
...
def preprocess_v1(
    ...
    # Mask targets  给 system message 和 question 都赋予 ignore_index label
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]  # 这里取 0 的原因是 pixel_values shape 为 [1, 3, H, W]
            else:
            ...
            if isinstance(video_file, str):
                video = os.path.join(video_folder, video_file)   # video_path
            ...
            sources = preprocess_multimodal(
                copy.deepcopy([e["conversations"] for e in sources]),
                self.data_args)
            sources = preprocess_target(sources, targets, self.data_args)
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'depth' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):  
                ...
                # batch['depths'] = depths  # list of (V, H, W)
                # batch['poses'] = poses  # list of (V, 4, 4)
                padded_depths = []
                padded_poses = []
...
def train(attn_implementation=None):
    ...
    # ======================================================================================
        ...
        if model_args.tune_mm_mlp_adapter:  # 这里是 freeze 所有 放开 mm_projector tune
            model.requires_grad_(False)
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = True
            # =========================================================================
            if model_args.tune_video_tower:
                for p in model.get_model().video_tower.parameters():  # 放开 video_tower
                    p.requires_grad = True 
            # =========================================================================                              
