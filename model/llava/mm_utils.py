...
def tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    ...
    # into two parts  len == 2
    # 1. A chat between a curious human and an artificial intelligence assistant. The assistant gives helpful, detailed, 
    # and polite answers to the human's questions.###Human: <image>

    # 2. \nWhat colors are the flowers surrounding the bench in the image?###Assistant: The flowers surrounding the 
    # bench in the image are yellow and purple.
    def insert_separator(X, sep):
        return [ele for sublist in zip(X, [sep]*len(X)) for ele in sublist][:-1]   # [[-200, -200], [-200, -200]]

    input_ids = []
    offset = 0
    # 这里是因为有一个 begin token 所以 offset +1
    # offset =1
    # prompt_chunks: [[1, 2555, ...], [1, 424, 2332, 213,... ]]
    # sep: [-200, -200]
    # [[1, 2555, ...], [-200, -200], [1, 424, 23332, 213, ...], [-200, -200]]
    # [[1, 2555, ...], [-200, -200], [1, 424, 23332, 213, ...]]
    # [2555, ...] + [-200] + [424, ...]
    ...
    return input_ids   # [1, 319, -200, ...., 13566, 29901]
