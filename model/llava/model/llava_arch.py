...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_rgbd_videos(self, images, depths, poses, intrinsics, lengths=None):
        ...
        image_features = self.get_model().get_vision_tower()(images.flatten(0, 1))  # (B*V, num_patch*num_patch [576], 1024)
        # print('image_feautures_is_nan:', torch.isnan(images).any())
        ...
        # print('image_shape:', image_features.shape)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, depths, poses, intrinsics, lengths, image_sizes=None
    ):
        vision_tower = self.get_vision_tower()
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels

        if images.ndim == 5 and depths is not None:  # (B, V, C, H, W)
            # (B, 1024, 576) or a list of tensor
            video_features, batch_offset = self.encode_rgbd_videos(images, depths, poses, intrinsics, lengths=lengths)
            # print('video_features_is_nan:', torch.isnan(image_features).any())
        ...
        # print('original_image_shape:', [x.shape[0] for x in image_features])
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):  # num_images = 1? [0, 1]
        ...
        # print('combined_embedding_shape:', [x.shape[0] for x in new_input_embeds])
