...
class ContrastiveEmbed(nn.Module):
    ...
    def forward(self,
        ...
        Args:
            ...
            pred_embeddings (Tensor): Text features.  # (b, text_lenth, text_dim)
        ...
        """
        res = visual_feat @ pred_embeddings.transpose(
            -1, -2)  # (b, num_query, 1)
...
class ReGround3DGroundingHead(BaseModule):
    """3D Grounding Head."""
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is ReGround3DGroundingHead):
    ...
    def _init_layers(self) -> None:
        ...
        if self.share_pred_layer:
            self.cls_branches = nn.ModuleList(
                [fc_cls for _ in range(1)])
            self.reg_branches = nn.ModuleList(
                [reg_branch for _ in range(1)])
        else:
            self.cls_branches = nn.ModuleList(
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        # and text tokens.
        labels = gt_bboxes.new_full((num_bboxes, ), 0, dtype=torch.float32)
        # (num_bboxes , max_text_len) gt_labels map
        labels[pos_inds] = 1
        labels = labels.unsqueeze(-1)  # (num_bboxes, 1)
        label_weights = gt_bboxes.new_ones((num_bboxes, 1))  # 这里每个 query 都被考虑进去了, 所以是个 contrastive loss
    ...
    def forward(
        ...
        Args:
            ...
            pred_embeddings (Tensor): Text feats. It has shape (bs, len_text,
                text_embed_dims).
        ...
        for layer_id in range(hidden_states.shape[0]):
            ...
            cls_scores = self.cls_branches[0](hidden_state, pred_embeddings)
    ...
    def predict(self, hidden_states: Tensor, all_layers_pred_bboxes: Tensor,
                pred_embeddings: Tensor, batch_data_samples: SampleList) -> InstanceList:
        ...
        Args:
            ...
            pred_embeddings (Tensor): Memory text. It has shape (bs, len_text,
                text_embed_dims).
        ...
        outs = self(hidden_states, pred_embeddings)
    ...
    def predict_by_feat(self, all_layers_cls_scores: Tensor,
                        ...
                        batch_gt_bboxes_3d: List) -> InstanceList:
    ...
    def _predict_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
                                gt_bboxes_3d: Tensor) -> InstanceData:
        ...
        cls_score = cls_score.sigmoid()  # (num_query, 1)
    ...
    def loss(self, hidden_states: Tensor, all_layers_pred_bboxes: Tensor,
             pred_embeddings: Tensor, batch_data_samples: SampleList) -> dict:
        """Perform forward propagation and loss calculation of the detection
        head on the queries of the upstream network.
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, pred_bboxes: Tensor,
        ...
        Returns:
            ...
            `loss_angle`.
        """
        batch_size = cls_scores.size(0)  # (bs, num_query)
        ...
        labels = torch.stack(labels_list, 0)  # (bs, num_query)
        label_weights = torch.stack(label_weights_list, 0)  # (bs, num_query)
        ...
        # print('label_weigths: shape:', label_weights.shape)
